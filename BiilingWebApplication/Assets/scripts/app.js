...
app = {
    ...
    live_count_down: function () {
        var deadline = new Date("Dec 2, 2019 09:00:00").getTime();
        var x = setInterval(function () {
            ...
            document.getElementById("live-count-down").innerHTML = "<div>" + days + "<span>days</span></div><div>"
            + hours + "<span>hours</span></div><div>" + minutes + "<span>minutes</span></div><div> " + seconds + "<span>seconds</span></div>";
            if (t < 0) {
                clearInterval(x);
                document.getElementById("live-count-down").innerHTML = "";
            }
        }, 1000);
    },
    ...
    masked_inputs: function (wrapper) {
        ...
        $currency = typeof wrapper != "undefined" ? $(wrapper).find('.mask-sales-currency') : $('.mask-sales-currency');
        if ($currency.length) {
            $currency.attr('data-inputmask', "'alias': 'currency', 'groupSeparator': ',', 'autoGroup': true, 'digits': 2, 'digitsOptional': false, 'prefix': '', 'placeholder': '0'");
            $currency.attr('data-inputmask-showmaskonhover', "false");
            $currency.inputmask();
